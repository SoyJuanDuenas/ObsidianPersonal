...
3. **Workspace:** In this panel, we can view the command history we have used, different variables, datasets, [[objects]], [[function]], and other elements we have declared.
...
To view our variable in the console (Panel #2), we can call it again, use the `print` [[CS/function]], or use a semicolon and call the [[CS/function]] on the same line.
...
To verify the type or class of an [[objects]], we can use the `class` [[CS/function]]. It may be necessary to convert an object from one type to another; for this, we use the appropriate [[CS/function]]:
...
- **Character:** [[vectors]] of characters, also known as strings.
...
A [[matrix]] is a two-dimensional type of [[data structures]] consisting of elements of the same type organized in rows and columns (bidimensional and homogeneous). It can be represented as a table or grid of numbers or values. Each element in the [[matrix]] is identified by its position determined by the row and column number.

To create matrices in R, we can do it directly or from previously created vectors. To create a [[matrix]] directly, we use:
...
Where the first parameter corresponds to the values to be used to fill the [[matrix]]. This interval must be a multiple of the number of rows or columns we want to create. The second parameter determines if we want to create the [[matrix]] by rows (`byrow = TRUE`) or by columns (`byrow = FALSE`).
...
Which, expressed in matrix form, is given by:
...
We can solve the system of equations in R as follows:
...
To access specific elements of the matrix, we use the index in the format (x, y) for (row, column):

```{r}
MAT2
# Show the element at row i, column j: MAT[i,j]
MAT2[2,1]
MAT2[1,2]
```

We can also use this to display only one row or one column of the matrix:

```{r}
MAT2
# Show the elements of row i: MAT[i,]
MAT2[1,]

# Show the elements of column j: MAT[,j]
MAT2[,1]
```

For a better understanding of our matrix, we can change the names of rows and columns, which will facilitate our subsequent work:
...
If we want to add columns and rows to our previously created matrices, we can use the `cbind()` and `rbind()` functions, respectively:
...
To remove rows, we use `[-i,]`, and to remove columns, we use the notation `[, -j]`:
